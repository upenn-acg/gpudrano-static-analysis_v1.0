...
__global__ void sobolGPU_kernel(unsigned n_vectors, unsigned n_dimensions, unsigned *d_directions, float *d_output)
{
    ...
    // block y coordinate
    d_directions = d_directions + n_directions * (blockIdx.y * blockDim.y + threadIdx.y);
    d_output = d_output +  n_vectors * (blockIdx.y * blockDim.y + threadIdx.y);
...
*/
    ...
    // from zero.
    unsigned int v_log2stridem1 = d_directions[__ffs(stride) - 2];
    ...
    for (unsigned int i = i0 + stride ; i < n_vectors ; i += stride)
    {
        ...
        X ^= v_log2stridem1 ^ d_directions[__ffs(~((i - stride) | v_stridemask)) - 1];
